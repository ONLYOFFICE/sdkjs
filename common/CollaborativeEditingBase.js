...
(function (window, undefined) {
  ...
  CCollaborativeChanges.prototype.Set_FromUndoRedo = function (
    Class,
    Data,
    Binary
  ) {
    if (!Class.Get_Id) return false;
  ...
  CCollaborativeChanges.prototype.Apply_Data = function () {
    ...
    if (fChangesClass) {
      ...
      if (!Class.Load_Changes) return false;
  ...
  CCollaborativeChanges.prototype.ToHistoryChange = function () {
    ...
    if (!AscDFH.changesFactory[changeType]) return null;
  ...
  CCollaborativeChanges.ToBase64 = function (pos, len) {
    ...
    if ((Asc.editor || editor).binaryChanges)
      return writer.GetDataUint8(pos, len);
    else return len + ";" + writer.GetBase64Memory2(pos, len);
  };
  ...
  CCollaborativeEditingBase.prototype.Remove_User = function (UserId) {
    ...
    if (-1 != Pos) this.m_aUsers.splice(Pos, 1);
  };
  ...
  CCollaborativeEditingBase.prototype.Apply_Changes = function (fEndCallBack) {
    ...
    if (this.m_aChanges.length > 0) {
      ...
      (
        Asc.editor || editor
      ).WordControl.m_oLogicDocument.EndPreview_MailMergeResult();

      (Asc.editor || editor).sync_StartAction(
        Asc.c_oAscAsyncActionType.BlockInteraction,
        Asc.c_oAscAsyncAction.ApplyChanges
      );
  ...
  CCollaborativeEditingBase.prototype.Apply_OtherChanges = function () {
    ...
    if (this.m_aChanges.length > 0) this.private_CollectOwnChanges();
  ...
  CCollaborativeEditingBase.prototype.GetNextExternalChange = function () {
    ...
    while (
      this.m_nExternalIndex1 >=
      this.m_aExternalChanges[this.m_nExternalIndex0].length
    ) {
      this.m_nExternalIndex0++;
      this.m_nExternalIndex1 = 0;

      if (this.m_nExternalIndex0 >= this.m_aExternalChanges.length) return null;
    }

    return this.m_aExternalChanges[this.m_nExternalIndex0][
      this.m_nExternalIndex1
    ];
  };
  /**
   ...
   */
  CCollaborativeEditingBase.prototype.ApplyExternalChanges = function (
    pointCount
  ) {
    if (!this.ValidateExternalChanges()) return 0;
  ...
  CCollaborativeEditingBase.prototype.CheckWaitingImages = function (
    aImages
  ) {};

  CCollaborativeEditingBase.prototype.SendImagesUrlsFromChanges = function (
    aImages
  ) {
    var rData = {},
      oApi = editor || Asc["editor"],
      i;
    if (!oApi) {
      return;
    }
    ...
    if (
      !oApi.CoAuthoringApi.callPRC(rData, Asc.c_nCommonRequestTime, callback)
    ) {
  ...
  CCollaborativeEditingBase.prototype.CollectImagesFromChanges = function () {
    ...
    for (i = 0; i < aNewImages.length; ++i) {
      ...
      if (sImageFromChanges.indexOf("theme") === 0 && oApi.ThemeLoader) {
        oThemeUrls[sImageFromChanges] =
          oApi.ThemeLoader.ThemesUrlAbs + sImageFromChanges;
      } else if (0 === sImageFromChanges.indexOf("data:")) {
        if (
          oApi.CoAuthoringApi &&
          oApi.CoAuthoringApi.get_standaloneApp &&
          oApi.CoAuthoringApi.get_standaloneApp()
        ) {
          aDataUrlForHost.push(sImageFromChanges);
        }
      } else if (
        0 === sImageFromChanges.indexOf("http:") ||
        0 === sImageFromChanges.indexOf("https:") ||
        0 === sImageFromChanges.indexOf("file:") ||
        0 === sImageFromChanges.indexOf("ftp:")
      ) {
      } else {
    ...
    //     var hex = Array.from(array, function (b) {
  ...
  CCollaborativeEditingBase.prototype.OnStart_Load_Objects = function (
    ...
    if (fEndCallBack) this.m_fEndLoadCallBack = fEndCallBack;
  ...
  CCollaborativeEditingBase.prototype.Set_GlobalLock = function (isLock) {
    if (isLock) this.m_bGlobalLock++;
    else this.m_bGlobalLock = Math.max(0, this.m_bGlobalLock - 1);
  };
  CCollaborativeEditingBase.prototype.Set_GlobalLockSelection = function (
    isLock
  ) {
    if (isLock) this.m_bGlobalLockSelection++;
    else
      this.m_bGlobalLockSelection = Math.max(
        0,
        this.m_bGlobalLockSelection - 1
      );
  };
  ...
  CCollaborativeEditingBase.prototype.OnEndCheckLockInstance = function () {
    var isLocked = false;
    for (
      var nIndex = 0, nCount = this.m_aCheckLocksInstance.length;
      nIndex < nCount;
      ++nIndex
    ) {
  ...
  //-----------------------------------------------------------------------------------
  CCollaborativeEditingBase.prototype.AddPosExtChanges = function (
    Item,
    ChangeObject
  ) {};
  CCollaborativeEditingBase.prototype.RefreshPosExtChanges = function () {};
  CCollaborativeEditingBase.prototype.RewritePosExtChanges = function (
    changesArr,
    scale,
    Binary_Writer
  ) {};
  ...
  CCollaborativeEditingBase.prototype.Clear_CollaborativeMarks = function (
    bRepaint
  ) {
  ...
  //----------------------------------------------------------------------------------------------------------------------
  CCollaborativeEditingBase.prototype.UpdateForeignCursorByAdditionalInfo =
    function (info) {
      if (!info) return;

      let userId = undefined !== info["UserId"] ? info["UserId"] : info.UserId;
      let cursorInfo =
        undefined !== info["CursorInfo"] ? info["CursorInfo"] : info.CursorInfo;
      let shortUserId =
        undefined !== info["UserShortId"]
          ? info["UserShortId"]
          : info.UserShortId;

      if (!userId || !cursorInfo || !shortUserId) return;

      this.Add_ForeignCursorToUpdate(userId, cursorInfo, shortUserId);
    };
  CCollaborativeEditingBase.prototype.Add_ForeignCursorToUpdate = function (
    UserId,
    CursorInfo,
    UserShortId
  ) {
  ...
  CCollaborativeEditingBase.prototype.private_UpdateForeignCursor = function (
    CursorInfo,
    UserId,
    Show,
    UserShortId
  ...
  CCollaborativeEditingBase.prototype.Refresh_ForeignCursors = function () {
    for (var UserId in this.m_aCursorsToUpdate) {
      ...
      this.private_UpdateForeignCursor(
        CursorInfo,
        UserId,
        false,
        this.m_aCursorsToUpdateShortId[UserId]
      );
  ...
  CCollaborativeEditingBase.prototype.Add_DocumentPosition = function (
    DocumentPos
  ) {
    this.m_aDocumentPositions.Add_DocumentPosition(DocumentPos);
  };
  CCollaborativeEditingBase.prototype.Remove_DocumentPosition = function (
    docPos
  ) {
    this.m_aDocumentPositions.Remove_DocumentPosition(docPos);
  };
  CCollaborativeEditingBase.prototype.Add_ForeignCursor = function (
    UserId,
    DocumentPos,
    UserShortId
  ) {
    this.m_aForeignCursorsPos.Remove_DocumentPosition(
      this.m_aCursorsToUpdate[UserId]
    );
  ...
  CCollaborativeEditingBase.prototype.Update_DocumentPositionsOnAdd = function (
    Class,
    Pos
  ) {
  ...
  CCollaborativeEditingBase.prototype.Update_DocumentPositionsOnRemove =
    function (Class, Pos, Count) {
      this.m_aDocumentPositions.Update_DocumentPositionsOnRemove(
        Class,
        Pos,
        Count
      );
      this.m_aForeignCursorsPos.Update_DocumentPositionsOnRemove(
        Class,
        Pos,
        Count
      );
    };
  CCollaborativeEditingBase.prototype.OnStart_SplitRun = function (
    SplitRun,
    SplitPos
  ) {
  ...
  CCollaborativeEditingBase.prototype.Update_DocumentPosition = function (
    DocPos
  ) {
    this.m_aDocumentPositions.Update_DocumentPosition(DocPos);
  };
  CCollaborativeEditingBase.prototype.Update_ForeignCursorPosition = function (
    UserId,
    Run,
    InRunPos,
    isRemoveLabel
  ) {};
  CCollaborativeEditingBase.prototype.Update_ForeignCursorsPositions =
    function () {};
  CCollaborativeEditingBase.prototype.Check_ForeignCursorsLabels = function (
    X,
    Y,
    PageIndex
  ) {
    ...
    for (var UserId in this.m_aForeignCursorsXY) {
      ...
      if (
        (true === Cursor.Transform &&
          Cursor.PageIndex === PageIndex &&
          Cursor.X0 - Px3 < X &&
          X < Cursor.X1 + Px3 &&
          Cursor.Y0 - Px3 < Y &&
          Y < Cursor.Y1 + Px3) ||
        (Math.abs(X - Cursor.X) < Px7 &&
          Cursor.Y - Px3 < Y &&
          Y < Cursor.Y + Cursor.H + Px3 &&
          Cursor.PageIndex === PageIndex)
      ) {
  ...
  CCollaborativeEditingBase.prototype.Show_ForeignCursorLabel = function (
    UserId
  ) {
    ...
    if (!this.m_aForeignCursorsXY[UserId]) return;

    var Cursor = this.m_aForeignCursorsXY[UserId];
    if (Cursor.ShowId) clearTimeout(Cursor.ShowId);
    ...
    var UserShortId = this.m_aForeignCursorsId[UserId]
      ? this.m_aForeignCursorsId[UserId]
      : UserId;
  ...
  CCollaborativeEditingBase.prototype.Add_ForeignCursorToShow = function (
    UserId
  ) {
    this.m_aForeignCursorsToShow[UserId] = true;
  };
  CCollaborativeEditingBase.prototype.Remove_ForeignCursorToShow = function (
    UserId
  ) {
    delete this.m_aForeignCursorsToShow[UserId];
  };
  CCollaborativeEditingBase.prototype.Add_ForeignCursorXY = function (
    UserId,
    X,
    Y,
    PageIndex,
  ...
  CCollaborativeEditingBase.prototype.Remove_ForeignCursorXY = function (
    UserId
  ) {
    if (this.m_aForeignCursorsXY[UserId]) {
  ...
  CCollaborativeEditingBase.prototype.private_RestoreDocumentState = function (
    DocState
  ) {
    var LogicDocument = (Asc.editor || editor).WordControl.m_oLogicDocument;
    if (true !== this.Is_Fast()) {
  ...
  CCollaborativeEditingBase.prototype.WatchDocumentPositionsByState = function (
    DocState
  ) {
    this.Clear_DocumentPositions();

    if (DocState.Pos) this.Add_DocumentPosition(DocState.Pos);
    if (DocState.StartPos) this.Add_DocumentPosition(DocState.StartPos);
    if (DocState.EndPos) this.Add_DocumentPosition(DocState.EndPos);

    if (DocState.AnchorPos) this.Add_DocumentPosition(DocState.AnchorPos);
  ...
  CCollaborativeEditingBase.prototype.UpdateDocumentPositionsByState =
    function (DocState) {
      if (DocState.Pos) this.Update_DocumentPosition(DocState.Pos);
      if (DocState.StartPos) this.Update_DocumentPosition(DocState.StartPos);
      if (DocState.EndPos) this.Update_DocumentPosition(DocState.EndPos);

      if (DocState.AnchorPos) this.Update_DocumentPosition(DocState.AnchorPos);
  ...
  CCollaborativeEditingBase.prototype.Update_ForeignCursorLabelPosition =
    function (UserId, X, Y, Color) {
      ...
      var Cursor = this.m_aForeignCursorsXY[UserId];
      if (!Cursor || !Cursor.ShowId) return;
      if (
        this.m_oLogicDocument &&
        this.m_oLogicDocument.IsFocusOnNotes &&
        this.m_oLogicDocument.IsFocusOnNotes()
      ) {
  ...
  CCollaborativeEditingBase.prototype.GetDocumentPositionBinary = function (
    oWriter,
    PosInfo
  ) {
  ...
  CCollaborativeEditingBase.prototype.private_CollectOwnChanges = function () {
    var StartPoint =
      null === AscCommon.History.SavedIndex
        ? 0
        : AscCommon.History.SavedIndex + 1;
    var LastPoint = -1;

    if (this.m_nUseType <= 0) LastPoint = AscCommon.History.Points.length - 1;
    else LastPoint = AscCommon.History.Index;
  ...
  CCollaborativeEditingBase.prototype.private_AddOverallChange = function (
    oChange,
    isSave
  ) {
    // Здесь мы должны смержить пришедшее изменение с одним из наших изменений
    for (
      var nIndex = 0, nCount = this.m_oOwnChanges.length;
      nIndex < nCount;
      ++nIndex
    ) {
      if (
        oChange &&
        oChange.Merge &&
        false === oChange.Merge(this.m_oOwnChanges[nIndex])
      )
        return false;
    }

    if (false !== isSave) this.CoHistory.AddChange(oChange);
  ...
  CCollaborativeEditingBase.prototype.Undo = function () {
    if (true === this.Get_GlobalLock()) return;
  ...
  CCollaborativeEditingBase.prototype.GetAllChangesCount = function () {
    return (
      this.CoHistory.GetChangeCount() -
      this.m_nUndoBeforeApply +
      this.m_aChanges.length
    );
  };
  CCollaborativeEditingBase.prototype.CanUndo = function () {
    return this.CoHistory.CanUndoOwn();
  };
  CCollaborativeEditingBase.prototype.private_RecalculateDocument = function (
    oRecalcData
  ) {};
  CCollaborativeEditingBase.prototype.private_SaveRecalcChangeIndex = function (
    isStart
  ) {
    if (isStart) this.m_nRecalcIndexStart = this.CoHistory.GetChangeCount();
    else this.m_nRecalcIndexEnd = this.CoHistory.GetChangeCount() - 1;
  };
  //----------------------------------------------------------------------------------------------------------------------
  ...
  CDocumentPositionsManager.prototype.Add_DocumentPosition = function (
    Position
  ) {
    this.m_aDocumentPositions.push(Position);
  };
  CDocumentPositionsManager.prototype.Update_DocumentPositionsOnAdd = function (
    Class,
    Pos
  ) {
    for (
      var PosIndex = 0, PosCount = this.m_aDocumentPositions.length;
      PosIndex < PosCount;
      ++PosIndex
    ) {
      var DocPos = this.m_aDocumentPositions[PosIndex];
      for (
        var ClassPos = 0, ClassLen = DocPos.length;
        ClassPos < ClassLen;
        ++ClassPos
      ) {
        ...
        if (
          ...
          (_Pos.Position > Pos ||
            (_Pos.Position === Pos &&
              !(Class instanceof AscCommonWord.ParaRun)))
        ) {
          _Pos.Position++;
          break;
        }
  ...
  CDocumentPositionsManager.prototype.Update_DocumentPositionsOnRemove =
    function (Class, Pos, Count) {
      for (
        var PosIndex = 0, PosCount = this.m_aDocumentPositions.length;
        PosIndex < PosCount;
  ...
  CDocumentPositionsManager.prototype.OnStart_SplitRun = function (
    SplitRun,
    SplitPos
  ) {
    this.m_aDocumentPositionsSplit = [];
    ...
    for (
      ...
      for (
        ...
        if (
          SplitRun === _Pos.Class &&
          _Pos.Position &&
          _Pos.Position >= SplitPos
        ) {
          this.m_aDocumentPositionsSplit.push({
            DocPos: DocPos,
            NewRunPos: _Pos.Position - SplitPos,
          });
        }
      }
    }
  ...
  CDocumentPositionsManager.prototype.OnEnd_SplitRun = function (NewRun) {
    ...
    for (
      var PosIndex = 0, PosCount = this.m_aDocumentPositionsSplit.length;
      PosIndex < PosCount;
      ++PosIndex
    ) {
      var NewDocPos = [];
      NewDocPos.push({
        Class: NewRun,
        Position: this.m_aDocumentPositionsSplit[PosIndex].NewRunPos,
      });
  ...
  CDocumentPositionsManager.prototype.Update_DocumentPosition = function (
    DocPos
  ) {
    // Смотрим куда мапится заданная позиция
    var NewDocPos = DocPos;
    for (
      var PosIndex = 0, PosCount = this.m_aDocumentPositionsMap.length;
      PosIndex < PosCount;
      ++PosIndex
    ) {
    ...
    if (
      NewDocPos !== DocPos &&
      NewDocPos.length === 1 &&
      NewDocPos[0].Class instanceof AscCommonWord.ParaRun
    ) {
    ...
    // Возможно ран с позицией переместился в другой класс
      ...
      DocPos.length > 0 &&
      DocPos[DocPos.length - 1].Class instanceof AscCommonWord.ParaRun
    ) {
  ...
  CDocumentPositionsManager.prototype.Remove_DocumentPosition = function (
    DocPos
  ) {
    for (
      var Pos = 0, Count = this.m_aDocumentPositions.length;
      Pos < Count;
      ++Pos
    ) {
  ...
  window["AscCommon"].FOREIGN_CURSOR_LABEL_HIDETIME =
