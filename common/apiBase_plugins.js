...
(function (window, undefined) {
   ...
   */
  Api.prototype["pluginMethod_GetVersion"] = function () {
   ...
   */
  Api.prototype["pluginMethod_AddOleObject"] = function (data) {
    return this.asc_addOleObject(data, true);
  };
   ...
   */
  Api.prototype["pluginMethod_EditOleObject"] = function (data) {
    return this.asc_editOleObject(data);
  };
  ...
  Api.prototype["pluginMethod_PasteHtml"] = function (htmlText) {
    if (!AscCommon.g_clipboardBase) return null;
    ...
    if (
      ...
      if (textPr && textPr.TextPr) {
        ...
        _elem.style.fontWeight =
          true === textPr.TextPr.Bold ? "bold" : "normal";
        _elem.style.fontStyle =
          true === textPr.TextPr.Italic ? "italic" : "normal";

        let _color = textPr.TextPr.Color;
        if (_color)
          _elem.style.color =
            "rgb(" + _color.r + "," + _color.g + "," + _color.b + ")";
        else _elem.style.color = "rgb(0,0,0)";
      }
      ...
      if (props && props.font) {
        ...
        _elem.style.fontStyle =
          true === props.font.italic ? "italic" : "normal";
      }
    }
  ...
  /**
   * Emulates an Enter key press in the document.
   * @memberof Api
   * @typeofeditors ["CDE", "CSE", "CPE"]
   * @alias InputEnter
   * @see office-js-api/Examples/Plugins/{Editor}/Api/Methods/InputEnter.js
   */
  Api.prototype["pluginMethod_InputKey"] = function (keyNumber) {
    if (!this.canEdit() || this.isPdfEditor() || !AscCommon.g_inputContext)
      return;

    // Emulate Enter key press (key code 13)
    AscCommon.g_inputContext.emulateKeyDownApi(keyNumber);
  };
    ...
    if (window["AscDesktopEditor"] && status != null && status != "") {
      ...
      if (!window["AscDesktopEditor"]["IsLocalFile"]()) {
        ...
        if (undefined !== window.LastUserSavedIndex) {
          ...
          else this.UpdateInterfaceState();
        }
      }
    ...
    switch (obj.type) {
      ...
      case "getPasswordByFile": {
        if ("" != obj["password"]) {
          var _param =
            "<m_sPassword>" +
            AscCommon.CopyPasteCorrectString(obj["password"]) +
            "</m_sPassword>";
  ...
  Api.prototype["pluginMethod_SetProperties"] = function (obj) {
    if (!this.isDocumentLoadComplete && obj) {
      if (!this.setPropertiesObj) {
        ...
        for (var item in obj) this.setPropertiesObj[item] = obj[item];
      }
      return;
    }
      ...
      switch (prop) {
        ...
        case "watermark_on_draw": {
          ...
          AscFonts.FontPickerByCharacter.checkText(sText, this, function () {
            this.watermarkDraw = obj[prop]
              ? new AscCommon.CWatermarkOnDraw(obj[prop], this)
              : null;
        ...
        case "fillForms": {
          ...
          if (oMap["tags"]) {
            ...
            for (let sTag in oMap["tags"]) {
              oTags[sTag] = [];
              for (
                let nIndex = 0, nCount = arrControls.length;
                nIndex < nCount;
                ++nIndex
              ) {
                let oForm = arrControls[nIndex];
                if (oForm && oForm.IsForm() && sTag === oForm.GetTag()) {
                  oTags[sTag].push(oForm);

                  let oElement;
                  if (oForm.IsInlineLevel()) oElement = oForm.GetParagraph();
                  else oElement = oForm;
            ...
            function FillForms() {
              if (
                ...
                for (let sTag in oTags) {
                  ...
                  for (
                    let nFormIndex = 0, nFormsCount = oTags[sTag].length;
                    nFormIndex < nFormsCount;
                    ++nFormIndex
                  ) {
  ...
  Api.prototype["pluginMethod_SetPluginsOptions"] = function (options) {
    ...
    if (!runObject.isConnector) return;
   ...
   */
  Api.prototype["pluginMethod_ShowInputHelper"] = function (
    guid,
    w,
    h,
    isKeyboardTake,
    ...
    if (w > _offset.W) w = _offset.W;
    if (h > _offset.H) h = _offset.H;
    ...
    if (_r > _offset.W) _x += _offset.W - _r;
    ...
    if (!this.isMobileVersion) _frame.style.zIndex = 1000;
    else _frame.style.zIndex = 5001;
  ...
  Api.prototype["pluginMethod_UnShowInputHelper"] = function (guid, isclear) {
    ...
    if (AscCommon.g_inputContext && AscCommon.g_inputContext.HtmlArea) {
      ...
      for (var test in AscCommon.g_inputContext.isInputHelpers) {
        if (AscCommon.g_inputContext.isInputHelpers[test]) count++;
      }
  ...
  Api.prototype["pluginMethod_GetSelectionType"] = function () {
    switch (this.editorId) {
      case AscCommon.c_oEditorId.Word: {
        ...
        if (!logicDoc.IsSelectionUse()) return "none";

        if (logicDoc.IsTextSelectionUse()) return "text";
      ...
      case AscCommon.c_oEditorId.Presentation: {
        ...
        if (-1 === logicDoc.CurPage) return "none";
      ...
      case AscCommon.c_oEditorId.Spreadsheet: {
        if (!this.wb || !this.wb.getWorksheet()) return "none";

        var objectRender = this.wb.getWorksheet().objectRender;
        if (!objectRender) return "none";
        ...
        if (!controller.IsSelectionUse() && !selection) retType = "none";
        if (controller.GetSelectionBounds() || selection) retType = "text";
        if (controller.getSelectedObjectsBounds()) retType = "drawing";
   ...
   */
  Api.prototype["pluginMethod_ConvertDocument"] = function (
    sConvertType,
    bHtmlHeadings,
    bBase64img,
    bDemoteHeadings,
  ...
  Api.prototype["pluginMethod_GetFileToDownload"] = function (format) {
    ...
    if (format && typeof format === "string") format = format.toUpperCase();

    let dwnldF =
      Asc.c_oAscFileType[format] ||
      Asc.c_oAscFileType[this.DocInfo.Format.toUpperCase()];
  ...
  function installPlugin(config, loadFuncName) {
    ...
    if (!currentInstalledPlugins) currentInstalledPlugins = {};
  ...
  Api.prototype["getUsedBackgroundPlugins"] =
    Api.prototype.getUsedBackgroundPlugins;
  ...
  Api.prototype.checkInstalledPlugins = function () {
    if (this.disableCheckInstalledPlugins) return;
    ...
    if (
      ...
      window["Asc"]["extensionPlugins"].length
    ) {
    ...
    if (currentRemovedPlugins) {
      for (let guid in currentRemovedPlugins) {
        if (guid) {
          if (window.g_asc_plugins.unregister(guid)) isRemovedPresent = true;
        }
      }
    }
  ...
  Api.prototype["pluginMethod_GetInstalledPlugins"] = function () {
    ...
    for (let i = 0, len = pluginsArray.length; i < len; i++) {
      if (pluginsArray[i].isConnector) continue;
      returnArray.push({
        baseUrl: baseUrl,
  ...
  Api.prototype["pluginMethod_RemovePlugin"] = function (guid, backup) {
    ...
    if (removedPlugin) {
      let currentRemovedPlugins = getLocalStorageItem("asc_plugins_removed");
      if (!currentRemovedPlugins) currentRemovedPlugins = {};
      ...
      if (
        ...
        currentInstalledPlugins[removedPlugin.guid]
      ) {
  ...
  Api.prototype["pluginMethod_OnDropEvent"] = function (obj) {
    ...
    switch (obj.type) {
      ...
      case "ondrop": {
        ...
        if (obj["html"]) this["pluginMethod_PasteHtml"](obj["html"]);
        else if (obj["text"]) this["pluginMethod_PasteText"](obj["text"]);
  ...
  Api.prototype["pluginMethod_GetDocumentLang"] = function () {
    ...
    if (
      this.WordControl &&
      this.WordControl.m_oLogicDocument &&
      this.WordControl.m_oLogicDocument.GetDefaultLanguage
    )
  ...
  function correctItemsWithData(items, baseUrl) {
    for (let i = 0, itemsLen = items.length; i < itemsLen; i++) {
      ...
      if (items[i]["items"]) correctItemsWithData(items[i]["items"], baseUrl);
    }
  }
  ...
  Api.prototype["pluginMethod_AddContentControlButtons"] = function (buttons) {
    if (AscCommon.c_oEditorId.Word !== this.getEditorId() || !buttons) return;
   ...
   */
  Api.prototype["pluginMethod_ResizeWindow"] = function (
    frameId,
    size,
    minSize,
    maxSize,
  ...
  Api.prototype["pluginMethod_onAIRequest"] = function (data) {
    ...
    if (this.aiResolvers.length > 0) this._AI();
