...
else
{
	...
	ZLib.prototype.getFile = function(path)
	{
		...
		// check - maybe we already uncompressed it?
	...
	ZLib.prototype.addFile = function(path, data)
	{
		...
		// check - maybe such file already exists? then it needs to be deleted first?
		if (undefined !== this.files[path])
	...
	ZLib.prototype.removeFile = function(path)
	{
		...
		// check - maybe such file doesn't exist?
		if (undefined === this.files[path])
			return false;
