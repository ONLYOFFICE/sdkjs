...
(function(){
    ...
    */
    function CAnnotationStamp(sName, nPage, aRect, oDoc)
    {
        ...
        this.sourceRect = aRect ? aRect.slice() : null; // исходный рект
        this.Init();
    }
    ...
    CAnnotationStamp.prototype.GetOriginViewScale = function() {
        ...
        let nSourceW = this.sourceRect[2] - this.sourceRect[0];
        let nCurrentW = this._origRect[2] - this._origRect[0];

        return nCurrentW / nSourceW;
    };
    ...
    CAnnotationStamp.prototype.SetRect = function(aOrigRect) {
        let oViewer     = editor.getDocumentRenderer();
    ...
    CAnnotationStamp.prototype.GetInnerRectangleSizes = function() {
        let nRot = this.GetRot();
        let aRect = this.GetRect();

        let W = aRect[2] - aRect[0];
        let H = aRect[3] - aRect[1];
        
        const w = W * Math.cos(nRot) + H * Math.sin(nRot);
        const h = H * Math.cos(nRot) + W * Math.sin(nRot);
    ...
    CAnnotationStamp.prototype.Init = function() {
        ...
        this.spPr.xfrm.setOffX(xMin);
        this.spPr.xfrm.setOffY(yMin);
        this.spPr.xfrm.setExtX(Math.abs(xMax - xMin));
        this.spPr.xfrm.setExtY(Math.abs(yMax - yMin));
