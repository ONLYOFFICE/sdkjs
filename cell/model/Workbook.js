...
//-------------------------------------------------------------------------------------------------
	 ...
	 */
	Worksheet.prototype.getColXf = function(index) {
		var attrArray = this.cellsXfByCol[index];
		if(!attrArray){
			attrArray = new AscCommonExcel.CAttrArray(null);
	...
	Worksheet.prototype.copyFrom=function(wsFrom, sName, tableNames){
		...
		wsFrom._forEachColXf(function(attrArray, index){
			t.cellsXfByCol[index] = attrArray.clone();
		});
	...
	Worksheet.prototype._forEachColXf = function(fAction) {
		for (var i = 0; i < this.cellsXfByCol.length; ++i) {
			var attrArray = this.cellsXfByCol[i];
			if (attrArray) {
				fAction(attrArray, i);
			}
		}
	...
	Worksheet.prototype._removeRows=function(start, stop){
		...
		for (let i = 0; i < this.cellsXfByCol.length; ++i) {
			...
			if (attrArray) {
				attrArray.deleteRange(start, -nDif);
			}
		}
	...
	Worksheet.prototype._insertRowsBefore=function(index, count){
		...
		for (let i = 0; i < this.cellsXfByCol.length; ++i) {
			let attrArray = this.cellsXfByCol[i];
			if (attrArray) {
				attrArray.insertRange(index, count);
			}
		}
		//copy property from row/cell above
		if (index > 0 && !this.workbook.bUndoChanges)
		{
			this.rowsData.setAreaByRow((index - 1), index, count);
			this.nRowsCount = Math.max(this.nRowsCount, this.rowsData.getMaxIndex() + 1);
			this._forEachColXf(function(attrArray) {
				attrArray.setAreaByRow((index - 1), index, count);
			});
			this.nRowsCount = Math.max(this.nRowsCount, this.cellsByColRowsCount);
			...
			for (let id in borders) {
				let nCol = parseInt(id);
				this.getRange3(index, nCol, index + count - 1, nCol).setBorderSrc(borders[nCol]);
			}
		}
		//notifyChanged after move cells to get new locations(for intersect ranges)
	...
	Worksheet.prototype._insertColsBefore=function(index, count){
		...
		if (!this.workbook.bUndoChanges) {
			...
			if (prevCellsByCol) {
				...
				//show rows and remain only cell xf property
				for (let id in borders) {
					let nRow = parseInt(id);
					this.getRange3(nRow, index, nRow, index + count - 1).setBorderSrc(borders[nRow]);
				}
			}
		}
	...
	Worksheet.prototype._shiftCellsRight=function(oBBox, displayNameFormatTable){
		...
		var cellsByColLength = this.getColDataLength();
		for (let i = cellsByColLength - 1; i >= nLeft; --i) {
			let sheetMemoryFrom = this.getColDataNoEmpty(i);
			if (sheetMemoryFrom) {
			...
			let attrArray = this.getColXfNoEmpty(i);
			if (attrArray) {
				if (i + dif <= gc_nMaxCol0) {
					this.getColXf(i + dif).copyRange(attrArray, oBBox.r1, oBBox.r1, oBBox.r2 - oBBox.r1 + 1);
				}
				attrArray.clear(oBBox.r1, oBBox.r2 + 1);
			}
		}
		this.setColsCount(Math.max(this.nColsCount, this.getColDataLength()));
		//copy property from row/cell above
		if (nLeft > 0 && !this.workbook.bUndoChanges)
		{
			...
			if (prevSheetMemory) {
				...
				for (var i = nLeft; i <= nRight; ++i) {
					...
					this.getColXf(i).copyRange(prevSheetMemory, oBBox.r1, oBBox.r1, oBBox.r2 - oBBox.r1 + 1);
				}
				this.setColsCount(Math.max(this.nColsCount, this.getColDataLength()));
				// var attrArray = this.worksheet.getColXfNoEmpty(i);
				// if (attrArray) {
				// 	attrArray.copyRange(attrArray, oBBox.r1, 0, height);
				// 	for (j in oSortedIndexes) {
				// 		nIndexFrom = j - 0;
				// 		nIndexTo = oSortedIndexes[j];
				// 		tempSheetMemory.copyRange(attrArray, nIndexFrom, nIndexTo - oBBox.r1, 1);
				// 	}
				// 	attrArray.copyRange(tempSheetMemory, 0, oBBox.r1, height);
				// }
				//show rows and remain only cell xf property
				for (let id in borders) {
					let nRow = parseInt(id);
					this.getRange3(nRow, oBBox.c1, nRow, oBBox.c2).setBorderSrc(borders[nRow]);
				}
			}
		}
		//notifyChanged after move cells to get new locations(for intersect ranges)
	...
	Worksheet.prototype._shiftCellsBottom=function(oBBox, displayNameFormatTable){
		...
		for (let i = 0; i < this.cellsXfByCol.length; ++i) {
			let attrArray = this.cellsXfByCol[i];
			if (attrArray) {
				attrArray.insertRange(nTop, dif);
			}
		}
		...
		if (nTop > 0 && !this.workbook.bUndoChanges)
		{
			for (var i = oBBox.c1; i <= oBBox.c2; ++i) {
				var sheetMemory = this.getColDataNoEmpty(i);
				if (sheetMemory) {
					sheetMemory.setAreaByRow((nTop - 1), nTop, dif);
					t.cellsByColRowsCount = Math.max(t.cellsByColRowsCount, sheetMemory.getMaxIndex() + 1);
				}
				let attrArray = this.getColXfNoEmpty(i);
				if (attrArray) {
					attrArray.setAreaByRow((nTop - 1), nTop, dif);
				}
			}
			this.nRowsCount = Math.max(this.nRowsCount, this.cellsByColRowsCount);
			//show rows and remain only cell xf property
			for (let id in borders) {
				let nCol = parseInt(id);
				this.getRange3(oBBox.r1, nCol, oBBox.r2, nCol).setBorderSrc(borders[nCol]);
			}
		}
		//notifyChanged after move cells to get new locations(for intersect ranges)
		this.workbook.dependencyFormulas.notifyChanged(renameRes.changed);
		...
		for(var i = 0; i < bordersArr.length; i++) {
			if(bordersArr[i]) {
				for(var j = 0; j < bordersArr[i].length; j++) {
					if(bordersArr[i][j]) {
						this.getCell3(i, j).setBorderSrc(bordersArr[i][j]);
					}
				}
			}
		}
...
//-------------------------------------------------------------------------------------------------
	...
	Cell.prototype.loadContent = function(row, col, opt_extMemory, opt_sheetMemory, opt_extXf, opt_xf) {
		var res = false;
		this.clear();
		this.nRow = row;
		this.nCol = col;
		var sheetMemory = opt_extMemory ? opt_sheetMemory : this.ws.getColDataNoEmpty(this.nCol);
		if (sheetMemory) {
		...
		if (opt_extXf) {
			this.xfs = opt_xf;
			res = res || null !== this.xfs;
		} else {
			let attrArray = this.ws.getColXfNoEmpty(this.nCol);
			if (attrArray) {
	...
	Cell.prototype.setXfId=function(val){
		...
		var oRes = this.ws.workbook.oStyleManager.setXfId(this, val);
		if(AscCommon.History.Is_On() && oRes.oldVal != oRes.newVal)
			AscCommon.History.Add(AscCommonExcel.g_oUndoRedoCell, AscCH.historyitem_Cell_XfId, this.ws.getId(), new Asc.Range(this.nCol, this.nRow, this.nCol, this.nRow), new UndoRedoData_CellSimpleData(this.nRow, this.nCol, oRes.oldVal, oRes.newVal));
	};
	...
	Cell.prototype.convertMultiText=function(){
			...
			if (valueCalc) {
				if (0 <= valueCalc.numFormat) {
					if (aStandartNumFormatsId[t.getNumFormatStr()] == 0) {
						t.ws.getCell3(t.nRow, t.nCol).setNum(new AscCommonExcel.Num({id: valueCalc.numFormat}));
					}
				} else if (typeof valueCalc.numFormat === "string") {
					t.ws.getCell3(t.nRow, t.nCol).setNum(new AscCommonExcel.Num({f: valueCalc.numFormat}));
	...
	Cell.prototype._updateCellValue = function() {
		...
		if (res) {
			...
			if(res.getHyperlink && null !== res.getHyperlink() && (!isArray || (isArray && firstArrayRef))) {
				...
				AscFormat.ExecuteNoHistory(function () {
					this.ws.getCell3(this.nRow, this.nCol).setFont(oHyperlinkFont);
				}, this, []);
			}
		...
		if(null != this.multiText && this.multiText.length > 0)
		{
			...
			if(bSetCellFont)
			{
				if (oIntersectFont.isEqual(g_oDefaultFormat.Font)) {
					this.ws.getCell3(this.nRow, this.nCol).setFont(null, true);
				} else {
					this.ws.getCell3(this.nRow, this.nCol).setFont(oIntersectFont, true);
				}
			}
...
//-------------------------------------------------------------------------------------------------
	...
	Range.prototype._foreachNoEmptyByCol = function(actionCell, excludeHiddenRows) {
		...
		if (actionCell && oBBox.c1 <= minC && oBBox.r1 <= minR) {
			...
			for (j = oBBox.c1; j <= minC; ++j) {
				...
				colXf = this.worksheet.getColXfNoEmpty(j);
				let xf = undefined;
				let colXfIter = colXf && new AscCommonExcel.CAttrArrayIterator(colXf, oBBox.r1, oBBox.r2, true);
				if (!(colXfIter && colXfIter.getMaxIndex() > 0)) {
					colXfIter = undefined;
				}
				if (colData || colXfIter) {
					let maxIndex = Math.max((colData && colData.getMaxIndex() || -1), (colXfIter && colXfIter.getMaxIndex() || -1));
					if (colXfIter) {
						colXfIter.next();
					}
					for (i = oBBox.r1; i <= Math.min(minR, maxIndex); i++) {
						if (bExcludeHiddenRows && this.worksheet.getRowHidden(i)) {
							excludedCount++;
							continue;
						}
						if (colXfIter && colXfIter.getCurFrom() <= i && i <= colXfIter.getCurTo()) {
							xf = colXfIter.getCurXf();
							if (!colXfIter.next()) {
								colXfIter = undefined;
							}
						}
						...
						if (null === targetCell) {
							if (tempCell.loadContent(i, j, true, colData, true, xf)) {
	...
	Range.prototype.setStyle=function(val){
		...
		//this._applyCellStyle(c_oRangeType.All === nRangeType, this.worksheet.workbook.oStyleManager.setNum, val, AscCH.historyitem_Cell_Num);
	};
	...
	Range.prototype.setFont=function(val, bKeepMultiText){
		AscCommon.History.Create_NewPoint();
		this.createCellOnRowColCross();
		var fSetProperty = this._setProperty;
		var nRangeType = this._getRangeType();
		...
		this._applyCellStyle(c_oRangeType.All === nRangeType, val, AscCH.historyitem_Cell_SetFont);
		if (!bKeepMultiText) {
			this.convertMultiText();
		}
	};
	...
	Range.prototype._applyCellStyleIter = function (colXf, colXfIter, val, historyType) {
		...
		if (AscCommon.History.Is_On() && oRes.oldVal !== oRes.newVal) {
			if (oRes.oldVal && oRes.oldVal.clone) {
				oRes.oldVal = oRes.oldVal.clone();
			}
			if (oRes.newVal && oRes.newVal.clone) {
				oRes.newVal = oRes.newVal.clone();
			}
			//todo to many objects
			let bbox = new Asc.Range(this.bbox.c1, colXfIter.getCurFrom(), this.bbox.c2, colXfIter.getCurTo());
			AscCommon.History.Add(AscCommonExcel.g_oUndoRedoCell, historyType, this.worksheet.getId(),
				bbox, new UndoRedoData_IndexSimpleProp(new AscCommonExcel.UndoRedoData_BBox(bbox), false, oRes.oldVal, oRes.newVal));
		}
	};
	Range.prototype._applyCellStyle = function (noEmpty, val, historyType) {
		if (noEmpty) {
			let colXfLen = this.worksheet.getColXfLength();
			for (let i = this.bbox.c1; i <= this.bbox.c2 && i < colXfLen; ++i) {
				let colXf = this.worksheet.getColXfNoEmpty(i);
				if (!colXf) {
					continue;
				}
				let colXfIter = new AscCommonExcel.CAttrArrayIterator(colXf, this.bbox.r1, this.bbox.r2, false, true);
				while (colXfIter.prevNoEmpty()) {
					this._applyCellStyleIter(colXf, colXfIter, val, historyType);
				}
			}
		} else {
			for (let i = this.bbox.c1; i <= this.bbox.c2; ++i) {
				let colXf = this.worksheet.getColXf(i);
				let colXfIter = new AscCommonExcel.CAttrArrayIterator(colXf, this.bbox.r1, this.bbox.r2, false, true);
				while (colXfIter.prev()) {
					this._applyCellStyleIter(colXf, colXfIter, val, historyType);
				}
			}
		}
	...
	Range.prototype.setAlignVertical=function(val){
		...
		this._applyCellStyle(c_oRangeType.All === nRangeType, val, AscCH.historyitem_Cell_AlignVertical);
	};
	Range.prototype.setAlignHorizontal=function(val){
		...
		this._applyCellStyle(c_oRangeType.All === nRangeType, val, AscCH.historyitem_Cell_AlignHorizontal);
	};
	Range.prototype.setFill=function(val){
		...
		this._applyCellStyle(c_oRangeType.All === nRangeType, val, AscCH.historyitem_Cell_Fill);
	};
	...
	Range.prototype.setBorderSrc=function(border){
		...
		this._applyCellStyle(c_oRangeType.All === nRangeType, border, AscCH.historyitem_Cell_Border);
		AscCommon.History.EndTransaction();
	};
	...
	Range.prototype._setCellBorder=function(bbox, cell, oNewBorder){
		if(oNewBorder)
		{
			...
			var nRow = cell.nRow;
			var nCol = cell.nCol;
			oNewBorder = this._setBorderMerge(nCol == bbox.c1, nRow == bbox.r1, nCol == bbox.c2, nRow == bbox.r2, oNewBorder, oCurBorder)
		}
		this.worksheet.getCell3(cell.nRow, cell.nCol).setBorderSrc(oNewBorder);
	};
	Range.prototype._setRowColBorder=function(bbox, rowcol, bRow, oNewBorder){
		...
		else
		{
			...
			oNewBorder = this._setBorderMerge(bLeft, bTop, bRight, bBottom, oNewBorder, oCurBorder)
			rowcol.setBorder(oNewBorder);
		}
	};
	Range.prototype._getBorderEdge=function(bbox, xfs, nRow, nCol, oNewBorder){
		let res = null;
		if (xfs && xfs.border)
		{
			var oCurBorder = xfs.border;
	...
	Range.prototype._setCellBorderIterCell = function (border, r1, c1, r2) {
		if (border) {
			let colXf = this.worksheet.getColXf(c1);
			let colXfIter = new AscCommonExcel.CAttrArrayIterator(colXf, r1, r2, false, true);
			while (colXfIter.prev()) {
				let xf = colXfIter.getCurXf();
				let oCurBorder = null;
				if (xf && xf.border) {
					oCurBorder = xf.border.clone();
				}
				let oNewBorder = this._setBorderMerge(c1 === this.bbox.c1, r1 === this.bbox.r1, c1 === this.bbox.c2, r2 === this.bbox.r2, border, oCurBorder)
				this._applyCellStyleIter(colXf, colXfIter, oNewBorder, AscCH.historyitem_Cell_Border);
			}
		} else {
			let colXf = this.worksheet.getColXfNoEmpty(c1);
			if (colXf) {
				let colXfIter = new AscCommonExcel.CAttrArrayIterator(colXf, r1, r2, true, true);
				while (colXfIter.prevNoEmpty()) {
					let xf = colXfIter.getCurXf();
					this._applyCellStyleIter(colXf, colXfIter, null, AscCH.historyitem_Cell_Border);
				}
			}
		}
	}
	Range.prototype._setCellBorderIterEdge = function (border, r1, c1, r2) {
		let colXf = this.worksheet.getColXfNoEmpty(c1);
		if (colXf) {
			let colXfIter = new AscCommonExcel.CAttrArrayIterator(colXf, r1, r2, true, true);
			while (colXfIter.prevNoEmpty()) {
				let xf = colXfIter.getCurXf();
				let newBorder = this._getBorderEdge(this.bbox, xf, r1, c1, border);
				this._applyCellStyleIter(colXf, colXfIter, newBorder, AscCH.historyitem_Cell_Border);
			}
		}
	}
	Range.prototype._setCellBorderIter = function (border) {
		var oBBox = this.bbox;
		let isInner =  null == border || (border.ih && !border.ih.isEmpty()) || (border.iv && !border.iv.isEmpty());
		let isLT = isInner || (border.l && !border.l.isEmpty()) || (border.t && !border.t.isEmpty());
		if (isLT) {
			this._setCellBorderIterCell(border, oBBox.r1, oBBox.c1, oBBox.r1);
		}
		let isRT = isInner || (border.r && !border.r.isEmpty()) || (border.t && !border.t.isEmpty());
		if (isRT) {
			this._setCellBorderIterCell(border, oBBox.r1, oBBox.c2, oBBox.r1);
		}
		let isRB = isInner || (border.r && !border.r.isEmpty()) || (border.b && !border.b.isEmpty());
		if (isRB) {
			this._setCellBorderIterCell(border, oBBox.r2, oBBox.c2, oBBox.r2);
		}
		let isLB = isInner || (border.l && !border.l.isEmpty()) || (border.b && !border.b.isEmpty());
		if (isLB) {
			this._setCellBorderIterCell(border, oBBox.r2, oBBox.c1, oBBox.r2);
		}
		let isL = isInner || (border.l && !border.l.isEmpty());
		if (isL) {
			this._setCellBorderIterCell(border, oBBox.r1 + 1, oBBox.c1, oBBox.r2 - 1);
		}
		let isT = isInner || (border.t && !border.t.isEmpty());
		if (isT) {
			for (let i = oBBox.c1 + 1; i <= oBBox.c2 - 1; ++i) {
				this._setCellBorderIterCell(border, oBBox.r1, i, oBBox.r1);
			}
		}
		let isR = isInner || (border.r && !border.r.isEmpty());
		if (isR) {
			this._setCellBorderIterCell(border, oBBox.r1 + 1, oBBox.c2, oBBox.r2 - 1);
		}
		let isB = isInner || (border.b && !border.b.isEmpty());
		if (isB) {
			for (let i = oBBox.c1 + 1; i <= oBBox.c2 - 1; ++i) {
				this._setCellBorderIterCell(border, oBBox.r2, i, oBBox.r2);
			}
		}
		if (isInner) {
			for (let i = oBBox.c1 + 1; i <= oBBox.c2 - 1; ++i) {
				this._setCellBorderIterCell(border, oBBox.r1 + 1, i, oBBox.r2 - 1);
			}
		}

		//убираем граничные border соседних ячеек
		if (oBBox.c1 > 0 && (null == border || (border.l && !border.l.isEmpty()))) {
			this._setCellBorderIterEdge(border, oBBox.r1, oBBox.c1 - 1, oBBox.r2);
		}
		if (oBBox.c2 < gc_nMaxCol0 && (null == border || (border.r && !border.r.isEmpty()))) {
			this._setCellBorderIterEdge(border, oBBox.r1, oBBox.c2 + 1, oBBox.r2);
		}
		if (oBBox.r1 > 0 && (null == border || (border.t && !border.t.isEmpty()))) {
			for (let i = oBBox.c1; i <= oBBox.c2; ++i) {
				this._setCellBorderIterEdge(border, oBBox.r1 - 1, i, oBBox.r1 - 1);
			}
		}
		if (oBBox.r2 < gc_nMaxRow0 && (null == border || (border.b && !border.b.isEmpty()))) {
			for (let i = oBBox.c1; i <= oBBox.c2; ++i) {
				this._setCellBorderIterEdge(border, oBBox.r2 + 1, i, oBBox.r2 + 1);
			}
		}
	};
	Range.prototype.setBorder=function(border){
		...
		for(var i = 0, length = aEdgeBorders.length; i < length; i++)
		{
			...
			range._setPropertyNoEmpty(function(row){
										  if(c_oRangeType.All == nRangeType && null == row.xfs)
											  return;
										  let newBorder = _this._getBorderEdge(oBBox, row.xfs, row.index, 0, border);
										  row.setBorder(newBorder);
									  },
									  function(col){
										  let newBorder = _this._getBorderEdge(oBBox, col.xfs, 0, col.index, border);
										  col.setBorder(newBorder);
									  });
		}
		this._setCellBorderIter(border);
	};
	Range.prototype.setShrinkToFit=function(val){
		...
		this._applyCellStyle(c_oRangeType.All === nRangeType, val, AscCH.historyitem_Cell_ShrinkToFit);
	};
	Range.prototype.setWrap=function(val){
		...
		this._applyCellStyle(c_oRangeType.All === nRangeType, val, AscCH.historyitem_Cell_Wrap);
	};
	Range.prototype.setAngle=function(val){
		...
		this._applyCellStyle(c_oRangeType.All === nRangeType, val, AscCH.historyitem_Cell_Angle);
	};
	...
	Range.prototype.setApplyProtection = function (val) {
		...
		this._applyCellStyle(c_oRangeType.All === nRangeType, val, AscCH.historyitem_Cell_SetApplyProtection);
	};
	Range.prototype.setLocked = function (val) {
		...
		this._applyCellStyle(c_oRangeType.All === nRangeType, val, AscCH.historyitem_Cell_SetLocked);
	};
	Range.prototype.setHiddenFormulas = function (val) {
		...
		this._applyCellStyle(c_oRangeType.All === nRangeType, val, AscCH.historyitem_Cell_SetHidden);
	};
...
// Узнаем отличается ли шрифт (размер и гарнитура) в ячейке от шрифта в строке
	...
	Range.prototype._sortByArray = function (oBBox, aSortData, bUndo, opt_by_row) {
		...
		var tempSheetMemory, tempAttrArray, nIndexFrom, nIndexTo, j;
		if (opt_by_row) {
			//data
			tempSheetMemory = [];
			for (j in oSortedIndexes) {
			...
			for (j in oSortedIndexes) {
				nIndexFrom = j - 0;
				nIndexTo = oSortedIndexes[j];
				var attrArrayFrom = this.worksheet.getColXf(nIndexFrom);
				var attrArrayTo = this.worksheet.getColXf(nIndexTo);

				tempAttrArray[nIndexTo] = new AscCommonExcel.CAttrArray(null);
				tempAttrArray[nIndexTo].copyRange(attrArrayTo, oBBox.r1, 0, height);

				if (tempAttrArray[nIndexFrom]) {
					attrArrayTo.copyRange(tempAttrArray[nIndexFrom], 0, oBBox.r1, height);
				} else {
					attrArrayTo.copyRange(attrArrayFrom, oBBox.r1, oBBox.r1, height);
				}
			}
		} else {
			//data
			...
			//style
			tempAttrArray = new AscCommonExcel.CAttrArray(null);
			for (i = oBBox.c1; i <= oBBox.c2; ++i) {
				var attrArray = this.worksheet.getColXfNoEmpty(i);
				if (attrArray) {
					tempAttrArray.copyRange(attrArray, oBBox.r1, 0, height);
					for (j in oSortedIndexes) {
						nIndexFrom = j - 0;
						nIndexTo = oSortedIndexes[j];
						tempAttrArray.copyRange(attrArray, nIndexFrom, nIndexTo - oBBox.r1, 1);
					}
					attrArray.copyRange(tempAttrArray, 0, oBBox.r1, height);
				}
			}
		}
...
// Перед promoteFromTo обязательно должна быть вызывана функция preparePromoteFromTo
		...
		for (let i = c1; i <= c2NoEmpty; i++) {
			...
			if (colXfIter) {
				if (r1 <= colXfIter.getMaxIndex() && colXfIter.next()) {
					let xf = colXfIter.getCurXf();
					elemAddXf = {col: i, index: Math.max(r1, colXfIter.getCurFrom()), type: "addXf", mod: xf, data: data};
					this.events.push(elemAddXf);
					let lastCurTo = colXfIter.getCurTo();
					while (colXfIter.next()) {
						let xf = colXfIter.getCurXf();
						this.events.push({col: i, index: colXfIter.getCurFrom(), type: "addXfMod", mod: xf, data: data});
						lastCurTo = colXfIter.getCurTo();
					}
					elemRemoveXf = {col: i, index: lastCurTo + 1, type: "removeXf", data: data};
	...
	SweepLineRowIterator.prototype.nextCol = function () {
		...
		let res = -1 !== this.col;
		if (res && null === this.sheetMemory && null === this.xf) {
			res = this.nextCol();
		}
		return res;
	};
	...
	RowIterator.prototype.next = function () {
		...
		while (this.iter.nextCol()) {
			...
			if (null === targetCell) {
				if (this.cell.loadContent(nRow, nCol, true, sheetMemory, true, xf)) {
					return this.cell;
				}
			} else {
				return targetCell;
