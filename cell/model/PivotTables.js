...
var GROUP__SECONDS_CAPTION = 'Seconds';
var GROUP__MINUTES_CAPTION = 'Minutes';
var GROUP__HOURS_CAPTION = 'Hours';
var GROUP__DAYS_CAPTION = 'Days';
var GROUP__MONTHS_CAPTION = 'Months';
var GROUP__QUARTERS_CAPTION = 'Quarters';
var GROUP__YEARS_CAPTION = 'Years';
...
CT_PivotCacheDefinition.prototype.groupRangePr = function(fld, rangePr) {
	var baseCacheField = this.getFields()[fld];
	var cacheField = new CT_CacheField();
	cacheField.name = this.cacheFields.generateNewName(baseCacheField.name);
	cacheField.numFmtId = 0;
	cacheField.databaseField = false;
	var fieldGroup = new CT_FieldGroup();
	fieldGroup.init(fld, baseCacheField, rangePr, dscretePr);
};
CT_PivotCacheDefinition.prototype.groupDiscrete = function(fld, groupMap) {
	var res = {addFields: [], reorderArray: null};
	var cacheFields = this.getFields();
	...
	if (!parCacheField) {
		...
		parCacheField.initGroupDiscrete(this.cacheFields.generateNewName(baseCacheField.name), fld, baseCacheField);
		cacheFields.push(parCacheField);

		var parFld = cacheFields.length - 1;
		baseCacheField.initGroupPar(parFld);

		res.addFields.push(parFld);
	}
	if (parCacheField.hasGroup()) {
		res.reorderArray = parCacheField.groupDiscrete(groupMap);
	}
	return res;
};
...
CT_PivotCacheDefinition.prototype.expand = function() {
	this.setPivotCacheId(AscCommon.CreateUInt32());
};
CT_PivotCacheDefinition.prototype.collapse = function() {
	this.setPivotCacheId(AscCommon.CreateUInt32());
};
...
CT_PivotCacheRecords.prototype._getDataMapFromFields = function(cacheFields, indexes, length, row, dataMap) {
	var i, index, val, cacheField, sharedIndex;
	for (i = 0; i < length; ++i) {
		index = indexes[i];
		if (index < cacheFields.length) {
			...
			if(cacheFieldBaseIndex < this.getColsCount()) {
				...
				sharedIndex = cacheField.getGroupOrSharedIndex(cacheFieldBase, val.val);
...
CT_PivotCacheRecords.prototype.getDataMap = function(cacheFields, filterMaps, cacheFieldsWithData, rowIndexes, colIndexes, dataFields) {
	...
	for (row = 0; row < this.getRowsCount(); ++row) {
		if (this.getDataMapLabelFilters(row, filterMaps, cacheFieldsWithData)) {
			continue;
		}
		this.fillVisibleFields(row, cacheFieldsWithData);
		rowMapCur = res;
...
CT_PivotCacheRecords.prototype.getDataMapLabelFilters = function(row, filterMaps, cacheFieldsWithData) {
	for (var i = 0; i < filterMaps.labelFilters.length; ++i) {
		...
		if (c_oAscPivotRecType.Index === val.type && !filter.map.has(val.val)) {
			if (cacheFieldsWithData[filter.index]) {
				//find other filters
				for (var j = i + 1; j < filterMaps.labelFilters.length; ++j) {
					var subFilter = filterMaps.labelFilters[j];
					var subVal = this._cols[subFilter.index].get(row);
					if (c_oAscPivotRecType.Index === subVal.type && !subFilter.map.has(subVal.val)) {
						return true;
					}
				}
				cacheFieldsWithData[filter.index][val.val] = 1;
			}
			return true;
		}
	}
...
CT_PivotCacheRecords.prototype.fillVisibleFields = function(row, cacheFieldsWithData) {
	for(var index in cacheFieldsWithData) {
		if (cacheFieldsWithData.hasOwnProperty(index)) {
			var visible = cacheFieldsWithData[index];
			var val = this._cols[index].get(row);
			if (c_oAscPivotRecType.Index === val.type && val.val < visible.length) {
				visible[val.val] = 1;
			}
		}
	}
...
CT_pivotTableDefinition.prototype._sortPivotItems = function(index, type, sortDataIndex) {
	...
	if (pivotField.sortType !== c_oAscFieldSortType.Manual && -1 === pivotField.getSortDataIndex()) {
		...
		pivotField.sortItems(type, cacheField.sharedItems);
		pivotField.checkSubtotal();
	}
};
...
CT_pivotTableDefinition.prototype.getLayoutsForGroup = function(selection) {
	var res = {fld: null, values: {}, valuesCount: 0}, i, layout, activeCell = selection.activeCell, cellLayout;
	...
	if (cellLayout && st_VALUES !== cellLayout.fld) {
		...
		res.valuesCount = Object.keys(res.values).length;
	}
	return res;
};
...
CT_pivotTableDefinition.prototype.groupRangePr = function(fld, groupMap) {
};
CT_pivotTableDefinition.prototype.groupDiscreteCache = function(fld, groupMap) {
	this.checkPivotFieldItems(fld);
	var groupMapCache = this._groupDiscreteCacheMap(fld, groupMap);
	return this.cacheDefinition.groupDiscrete(fld, groupMapCache);

};
CT_pivotTableDefinition.prototype.groupDiscrete = function(fld, groupMap, groupRes) {
	this.checkPivotFieldItems(fld);
	var pivotFields = this.asc_getPivotFields();
	var pivotField = pivotFields[fld];
	// pivotField.groupDiscrete(groupMap, groupRes.reorderArray);
	this._groupDiscreteAddFields(fld, groupMap, groupRes);
};
CT_pivotTableDefinition.prototype._groupDiscreteCacheMap = function(fld, groupMap) {
	var groupMapCache = {};
	var pivotFields = this.asc_getPivotFields();
	var pivotField = pivotFields[fld];
	if(!pivotField) {
	...
	for (var index in groupMap) {
		if (groupMap.hasOwnProperty(index)) {
			var item = pivotField.getItem(parseInt(index));
			groupMapCache[item.x] = 1;
		}
	}
	return groupMapCache;
};
CT_pivotTableDefinition.prototype._groupDiscreteAddFields = function(fld, groupMap, groupRes) {
	var cacheFields = this.asc_getCacheFields();
	var pivotFields = this.asc_getPivotFields();
	for (var i = 0; i < groupRes.addFields.length; ++i) {
		var newPivotField = new CT_PivotField();
		newPivotField.init(cacheFields[cacheFields.length-1].getGroupOrSharedSize());
		newPivotField.groupDiscrete(groupMap, groupRes.reorderArray);
		pivotFields.push(newPivotField);
		var pivotIndex = pivotFields.length - 1;
		var insertIndex = this.rowFields.find(fld);
		if(-1 != insertIndex) {
			this.addRowField(pivotIndex, insertIndex, true);
		}
		insertIndex = this.colFields.find(fld);
		if(-1 != insertIndex) {
...
CT_CacheField.prototype.getGroupType = function () {
	if (this.sharedItems && false === this.sharedItems.containsMixedTypes) {
		if (this.sharedItems.containsNumber) {
			return c_oAscGroupType.Number;
		} else if (this.sharedItems.containsDate) {
...
CT_PivotField.prototype.groupDiscrete = function(groupMap, reorderArray) {
	var newItem;
	...
	if (items) {
		...
		var isFirst = true;
		for (var i = 0; i < items.length; ++i) {
			if (groupMap[items[i].x]) {
				if (isFirst) {
					isFirst = false;
					newItem = new CT_Item();
					newItem.x = reorderArray[items[i].x];
...
CT_FieldGroup.prototype._groupDiscrete = function(groupMap) {
	...
	var newIndex = 1;
	while (groupNameMap.has(GROUP_TEXT_CAPTION + newIndex)) {
		newIndex++;
	}
	item = new PivotRecordValue();
	item.init(c_oAscPivotRecType.String, GROUP_TEXT_CAPTION + newIndex);
	newGroupItems.addItem(item);
...
CT_RangePr.prototype.readAttributes = function(attr, uq) {
	if (attr()) {
		...
		if (undefined !== val) {
			var d = new Asc.cDate(uq(val));
			this.startDate = new Asc.cDate(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate(), d.getHours(), d.getMinutes(),
				d.getSeconds(),	d.getMilliseconds()));
		}
		val = vals["endDate"];
		if (undefined !== val) {
			var d = new Asc.cDate(uq(val));
			this.endDate = new Asc.cDate(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate(), d.getHours(), d.getMinutes(),
				d.getSeconds(),	d.getMilliseconds()));
		}
	...
	if (this.groupBy === c_oAscGroupBy.Range) {
		...
		if(val < this.startDate) {
			...
			if (this.groupBy === c_oAscGroupBy.Seconds) {
				...
				//c_oAscGroupBy.Days
				if (1 === this.groupInterval) {
					res = Math.floor((val.getTime() - Date.UTC(val.getUTCFullYear())) / AscCommonExcel.c_msPerDay);
				} else {
					res = Math.floor((val.getTime() - this.startDate.getTime()) / AscCommonExcel.c_msPerDay / this.groupInterval);
				}
			}
...
CT_DiscretePr.prototype.group = function (reorderArray) {
	for(var i = 0; i < this.x.length; ++i) {
		this.x.v = reorderArray[this.x.v];
	}
};
...
PivotRecords.prototype.addRecordValue = function(record) {
	this._add(record.type, record.val, record.addition);
};
