...
// Table of ASCII code correspondences (decimal, corresponding octal, hexadecimal, binary, ASCII codes)
...
function CMathBaseText()
{
    ...
    this.Width          = 0; // special for Run: letter size without Gaps
                             // in reality this field should not be used, use functions GetWidth, Get_Width_2, GetWidthVisible instead
...
CMathBaseText.prototype.GetWidth = function() // working through function because GapLeft and GapRight fields may change due to line break changes, and recalculation (Measure) won't come in this case
{
...
CMathBaseText.prototype.Get_Width2 = function() // working through function because GapLeft and GapRight fields may change due to line break changes, and recalculation (Measure) won't come in this case
{
...
CMathBaseText.prototype.Draw_Elements = function(PDSE)  // this function is necessary for Draw_Elements of the Nary base when the base contains only a JustDraw element
{
...
CMathText.prototype.Measure = function(oMeasure, TextPr, InfoMathText)
{
	...
	*/
	...
	if(this.bJDraw)
	{
		// Font is set in the SetFont function of the parent class (in the general case in CMathBase)
	...
	else
	{
		...
		this.FontSlot = InfoMathText.GetFontSlot(letter); // returns AscWord.fontslot_ASCII || AscWord.fontslot_EastAsia || AscWord.fontslot_CS || AscWord.fontslot_HAnsi

		// in non-mathematical text, i and j are not replaced with i and j without dots
	...
	if(this.RecalcInfo.bApostrophe)
	{
		...
		this.rasterOffsetY = metricsA.Height - metricsTxt.Ascent; // offset for position
...
CMathText.prototype.Draw = function(x, y, pGraphics, InfoTextPr)
{
	...
	if(this.bJDraw)
	{
		pGraphics.FillTextCode(X, Y, this.RecalcInfo.StyleCode);    // send position baseline to draw symbol
	}
	else if(this.RecalcInfo.bSpaceSpecial == false)
	{
		...
		else
		{
			...
			{
				// send position baseline to draw symbol
...
CMathText.prototype.setCoeffTransform = function(sx, shx, shy, sy)
{
    ...
    // here we need to recalculate the size after transformation, the size will be set in g_oTextMeasurer
    //
...
{
    // Write type twice, once for the general reading function, and again to distinguish between ordinary MathText and PlaceHolder
...
CMathAmp.prototype.Draw = function(x, y, pGraphics, InfoTextPr)
{
    ...
    else if(Asc.editor.ShowParaMarks) // show alignment marks if paragraph marks are enabled
    {
...
{
    this.CurrType         = -1; // Font is always set for the first time
    ...
    // copy these properties for SetFontSlot
    // for SpecialOperator, we need already compiled text settings for math text, so it's essential to copy these properties here, rather than passing regular text settings to MathText
...
CMathInfoTextPr.prototype.IsSpecilalOperator = function(val)
{
    ...
    // symbols in the interval val >= 0x2E2 && val <= 0x36F are usually in fonts and are used exactly as they are, rather than being replaced with symbols from Cambria Math
